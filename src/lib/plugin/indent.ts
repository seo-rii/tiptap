...
import type {CommandProps, Extensions, KeyboardShortcutCommand} from '@tiptap/core'
    ...
    doc.nodesBetween(from, to, (node, pos) => {
        if (options.names.includes(node.type.name)) {
            tr = setNodeIndentMarkup(
                tr, pos, options.indentRange * (type === 'indent' ? 1 : -1),
                options.minIndentLevel, options.maxIndentLevel
            )
            return false
        }
...
const indent: () => KeyboardShortcutCommand = () => ({editor}) => {
    if (!isList(editor.state.doc.type.name, editor.extensionManager.extensions)) return editor.commands.indent()
    return false
}
const outdent: (outdentOnlyAtHead: boolean) => KeyboardShortcutCommand = (outdentOnlyAtHead) => ({editor}) => {
    if (!(
        isList(
